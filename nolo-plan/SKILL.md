...
2. **选通道**:适合 plan 时,**同一句话里报出执行通道**——查 nolo 指派表选执行者(见第 0 步)。没装 nolo CLI → 先走 setup 引导;装不了或不可用就按下方算账降级,不阻塞。
...
`which nolo` 不存在 → 走 setup 引导(见下方)。装不了或不可用时按派发前算账降级为宿主 subagent 或直接执行,不阻塞任务。
...
**硬规则:plan 写完 → 实现类 task 默认派发,不许规划者自己 edit。** 唯一豁免是强制门的派发前算账判定自己做更便宜(含微小豁免、紧急解阻),需一句理由;豁免情形下规划者自己写的 diff 仍受第 3 步「作者回避」约束,必须派另一个 agent review。
